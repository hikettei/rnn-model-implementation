...
(defun train (data-type lang1 lang2)
  (let ((max-length (max (calc-max-length :tok data-type :en)
    ...
    (multiple-value-bind (lang1-w2i lang1-i2w lang2-w2i lang2-i2w)
	(mvalues (collect-tokens data-type lang1)
		 (collect-tokens data-type lang2))
      
      (unless (= data-size (calc-data-size :tok data-type lang2))
	(error "Incorresponding the number of data-size between lang1 and lang2"))

      (multiple-value-bind (train-x train-y) (init-train-datas data-type lang1 lang2 lang1-w2i lang2-w2i data-size)
